        ...
        <div className="flex flex-col sm:flex-row flex-wrap gap-3 mb-6">
            {user?.role !== "student" && (
                <select
                    ...
                    className="w-full sm:w-auto px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white focus:ring-2 focus:ring-accent focus:outline-none"
                >
            ...
            {Object.values(filters).some(Boolean) && (
                <button
                    onClick={() => onFilter({})}
                    className="w-full sm:w-auto px-3 py-2 text-sm text-gray-500 hover:text-red-600 border border-gray-200 rounded-lg bg-white transition"
                >
                    Clear filters ✕
                </button>
            )}
