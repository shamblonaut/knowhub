        ...
        <Layout>
            <div className="flex flex-col lg:flex-row gap-4" style={{ height: 'calc(100vh - 120px)' }}>
                ...
                <div className="flex-1 flex flex-col min-w-0 h-full">
                            ...
                            <div className="flex items-center gap-3">
                                {allSources.length > 0 && (
                                    <button
                                        ...
                                        className="lg:hidden text-xs text-accent font-semibold border border-accent/20 px-2 py-1 rounded"
                                    >
                                        {showMobileSources ? 'Show Chat' : `Sources (${allSources.length})`}
                                    </button>
                                )}
                        ...
                        <div className="flex gap-2 flex-wrap">
                            {user?.role !== 'student' && (
                                <select value={semester} onChange={e => { setSemester(e.target.value); setSubjectId('') }}
                                    className="flex-1 sm:flex-initial px-3 py-1.5 border border-gray-200 rounded-lg text-xs bg-white focus:ring-1 focus:ring-accent focus:outline-none">
                            ...
                            <select value={subjectId} onChange={e => setSubjectId(e.target.value)} disabled={!effectiveSemester}
                                className="flex-1 sm:flex-initial px-3 py-1.5 border border-gray-200 rounded-lg text-xs bg-white focus:ring-1 focus:ring-accent focus:outline-none disabled:opacity-40">
                    ...
                    {/* Messages & Mobile Sources Toggle */}
                    <div className="flex-1 relative overflow-hidden flex flex-col">
                        {showMobileSources ? (
                            <div className="flex-1 overflow-y-auto bg-gray-50/50 rounded-xl p-4 lg:hidden">
                            ...
                            <div className="flex-1 overflow-y-auto px-1 pb-2">
                                {messages.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center h-full text-center py-10">
                                        ...
                                        <h2 className="text-lg font-bold text-primary mb-6 px-4">Ask your course material anything</h2>
                    ...
                    <form onSubmit={submit} className="flex gap-2 shrink-0 pt-3">
                        ...
                        <button type="submit" disabled={isLoading || !input.trim()}
                            className="w-12 h-12 bg-primary text-white rounded-xl text-xl font-bold hover:bg-opacity-90 disabled:opacity-40 shrink-0">
                            {isLoading ? '…' : '↑'}
                        </button>
                    </form>
                </div>
                ...
                {allSources.length > 0 && (
                    <div className="hidden lg:block w-72 shrink-0 border-l border-gray-100 bg-gray-50/50 overflow-y-auto rounded-r-xl h-full">
                        <div className="p-4">
                            <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4 px-1">References ({allSources.length})</h3>
                            <div className="space-y-3 pb-4">
                                {allSources.map((s, i) => <SourceCard key={`${s.resource_id}-${i}`} source={s} index={i + 1} />)}
